...
// https://developer.mozilla.org/en-US/Add-ons/SDK/Tools/cfx_to_jpm --static-args no longer works as of Firefox 44
ss.storage['devmode'] = (self.version.indexOf('-dev')>0
|| self.version.indexOf('-beta')>0
|| (system.staticArgs && system.staticArgs.dev));
...
const pagestatsapi = require("./pagestatsapi");
		...
		var create_pagemods = function() {
			...
			var loadPltMod = function() {
				...
				pltmods.push(pageMod.PageMod({
					include: ["*"],
					/*
					 * FIXME : if we can turn PAGESTATSJSURL into a template with variables and conditionals,
					 * we can enhance the way it works
					 * for example by detecting if the beforeunload event is disabled, we can use the unload event instead
					 */
					contentScriptFile: [consts.PAGELOADJSURL, consts.PAGESTATSJSURL],
					...
					onAttach: function(worker) {
						if (worker && worker.tab) {
							...
							/* local measurements per website's visite */
							worker.port.on("locallySavePerf", function(p){
								pagestatsapi.locallySavePerf(p);
							});
