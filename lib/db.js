...
/**
 * @fileoverfiew Sqlite DB handler.
 *
 * Fathom uses a sqlite DB for storing a local history of the baseline
 * measurements for the monitoring graphs and other info such as environments.
 *
...
DB.prototype.connect = function(cb) {
    ...
    Task.spawn(function() {
        ...
        if (!initdone) {
            ...
            yield that.conn.execute(SQL_CREATE_HTML_REQ);
            ...
            if (that.version < 7 || (that.version < 8 && utils.isWin())) {
                ...
                console.log('baselinedb tables re-created schema='+SCHEMA_VERSION);

            } else if (that.version < 9) {
                // mlab server info update
                yield that.conn.execute("ALTER TABLE env ADD COLUMN mserver_ip text;")
                yield that.conn.execute("ALTER TABLE env ADD COLUMN mserver_city text;")
                yield that.conn.execute("ALTER TABLE env ADD COLUMN mserver_country text;")
                console.log('baselinedb tables updated to schema='+SCHEMA_VERSION);
            }
            if (that.version < 10) {
                yield that.conn.execute(SQL_CREATE_HTML_REQ);
            }
        ...
        function() { if (cb) cb(); }, // success
        function(err) { // error handler
            that.close();
            console.error('baselinedb',err);
            if (cb) cb(error("dbconnfailed",err));
        }
    ); // Task
};
...
// row represents an main request
const SQL_CREATE_HTML_REQ = "CREATE TABLE IF NOT EXISTS htmlreq(rowid integer primary key not null, ts integer, timezoneoffset integer, size integer, navigation text, timing text, resourcetiming text, protocol text, host text, origin text, name text, extensions text, baselines text, numCookiesPerHost text)";

// insert a request
const SQL_INSERT_HTML_REQ = "INSERT INTO htmlreq(ts, timezoneoffset, size, navigation, timing, resourcetiming, protocol, host, origin, name, extensions, baselines, numCookiesPerHost) VALUES(:ts, :timezoneoffset, :size, :navigation, :timing, :resourcetiming, :protocol, :host, :origin, :name, :extensions, :baselines, :numCookiesPerHost)";

// get a range of baseline measurements
const SQL_SELECT_BASELINE = "SELECT * FROM baseline WHERE ts >= :tsmin AND ts <= :tsmax ORDER BY ts";
...
DB.prototype.saveBaselineRow = function(o, cb) {
    ...
    o.rowid = (rowid + 1) % config.get("baseline", "max_rows")[0];
...
/** Store an HTML request */
DB.prototype.saveHTMLRequest = function(req) {
    // HTML request row object
    var o = {
        /*rowid : null,*/// this line is omitted on purpose to use the sqlite automatic selection of the primary key
        ts : req.ts,//integer
        timezoneoffset : req.timezoneoffset,//integer
        size : req.size,//integer
        navigation : JSON.stringify(req.performance.navigation),//text
        timing : JSON.stringify(req.performance.timing),//text
        resourcetiming : JSON.stringify(req.performance.resourcetiming),//text
        protocol : req.protocol,//text
        host : req.location.host,//text
        origin : req.location.origin,//text
        name : req.location.name,//text
        extensions : JSON.stringify(req.extensions),//text
        baselines : JSON.stringify(req.baselines),//text
        numCookiesPerHost : JSON.stringify(req.numCookiesPerHost)//text
    };
    this.saveHTMLRequestRow(o);
};

/** The HTML request row save. */
DB.prototype.saveHTMLRequestRow = function(o, cb) {
  if (!this.isConnected()) {
      if (cb) cb(false);
      return;
  }
  ...
  Task.spawn(function* saveRound() {
      try {
          /* o is the object properly formatted with the right properties */
          yield that.conn.execute(SQL_INSERT_HTML_REQ,o);
          if (cb) cb(true);
      } catch(err) {
          console.error("HTML request save error",err);
          if (cb) cb(false);
      }
  });
}
    ...
    Task.spawn(function* agg() {
        try {
            ...
            for (var level = 1; level <= 3; level += 1) {
                ...
                let wend = wstart + cfg['intervals'][level]*1000;
                while (wend < ts) {
                    ...
                    for (let i = 0; i < rows.length; i++) {
                        ...
                        rowid = rowid % cfg['max_rows'][level];
                        oo.rowid = rowid;
                    ...
                    // next window
                    wstart = wend;
                    wend = wstart + cfg['intervals'][level]*1000;
                } // while
...
const SQL_CREATE_ENV = "CREATE TABLE IF NOT EXISTS env(rowid integer primary key autoincrement, first_seen_ts integer, last_seen_ts integer, default_iface_name text, default_iface_mac text, gateway_ip text, gateway_mac text, ssid text, bssid text, hop1_ip text, hop2_ip text, hop3_ip text, userlabel text unique, public_ip text, country text, city text, isp text, net_desc text, as_number text, as_desc text, lookup_ts integer, mserver_ip text, mserver_city text, mserver_country text)";
...
const SQL_INSERT_ENV = "INSERT INTO env(first_seen_ts, default_iface_name, default_iface_mac, gateway_ip, gateway_mac, ssid, bssid) VALUES(:first_seen_ts, :default_iface_name, :default_iface_mac, :gateway_ip, :gateway_mac, :ssid, :bssid)";
...
DB.prototype.updateEnv = function(env, cb) {
    ...
    Task.spawn(function* updateEnv() {
        yield that.conn.execute(
            ...
            "', as_desc='"+env.as_desc +
            "', mserver_ip='"+env.mserver_ip +
            "', mserver_country='"+env.mserver_country +
            "', mserver_city='"+env.mserver_city +
...
DB.prototype.getEnvRange = function(range, cb) {
    ...
    var stmt = "SELECT s.ts as ts, e.rowid as env_id, e.first_seen_ts, e.last_seen_ts, "+
        ...
        "e.userlabel, e.net_desc, e.as_number, e.as_desc, e.lookup_ts, e.mserver_ip, e.mserver_city, e.mserver_country FROM "+t+
...
/** Get timeseries of selected baseline metrics. */
DB.prototype.getBaselineByTimestamp = function(p,cb) {
    if (!this.isConnected())
      return cb(error("internal","db is not connected"));
    let baselines = [];
    /* rowid is ignored on purpose */
    let cols = ['env_id', 'ts', 'tasks_total', 'tasks_running', 'tasks_sleeping', 'loadavg_onemin', 'loadavg_fivemin', 'loadavg_fifteenmin', 'cpu_user', 'cpu_system', 'cpu_idle', 'mem_total', 'mem_used', 'mem_free', 'mem_ff', 'wifi_signal', 'wifi_noise', 'wifi_quality', 'rx', 'tx', 'rtt0', 'rtt1', 'rtt2', 'rtt3', 'rttx', 'pageload_total', 'pageload_dns', 'pageload_firstbyte', 'pageload_total_delay', 'pageload_dns_delay', 'pageload_firstbyte_delay'];
    /* we gather every measurement between the beginning of the page and its end */
    let range = {tsmin:p.performance.timing.navigationStart,tsmax:p.ts};
    // https://developer.mozilla.org/en-US/docs/Mozilla/JavaScript_code_modules/Sqlite.jsm
    this.conn.execute(SQL_SELECT_BASELINE,range,function(row) {
      let o = {};
      _.each(cols, function(col) {
          o[col] = row.getResultByName(col);
      });
      baselines.push(o);
    }).then(function onStatementComplete(result) { /* if the function above is a success we add the baselines measurements */
      p.baselines = baselines;
      console.debug('the call of saveHTMLRequest starts here');
      getInstance().saveHTMLRequest(p);
      console.debug('the call of saveHTMLRequest ends here');
    },
    function onError(err) { /* if the function above has failed, we ignore baselines measurements */
      console.log(err);
      p.baselines = null;
      console.debug('the call of saveHTMLRequest starts here');
      getInstance().saveHTMLRequest(p);
